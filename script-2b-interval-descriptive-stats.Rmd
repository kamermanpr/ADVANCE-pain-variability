...
df <- read_rds('data-cleaned/data-ADVANCE.rds') %>% 
    select(ranid, interval_name, pain_in_the_last_week, 
           cd4_cells.ul, viral_load_cp.ml, tb_screen, mms_total,
...
```{r tb}
# Tabulate data
as.data.frame(xtabs(~interval_name + tb_screen, 
                               data = df)) %>% 
    rename(count = Freq) %>% 
    group_by(interval_name) %>% 
...
Values $\geq$ 6 indicate a need to full psychiatric evaluation.

```{r mms}
# Tabulate data
df %>% 
  select(interval_name, mms_total) %>% 
  group_by(interval_name) %>% 
  skim_to_wide() %>% 
  select(-type, -variable) %>% 
  skimr::kable(caption = '5-number summary of the MMS total score')
...
df %>% 
  ...
  aes(x = factor(interval_numeric),
      y = mms_total) +
  geom_boxplot() +
  labs(subtitle = 'Modified Mini Score',
       y = 'MMS score',
       x = 'Weeks')
